// ============================= LEARNING NOTES =============================
// CONTENT COLLECTIONS: Astro's way of managing groups of similar content
// (blog posts, help articles, glossary terms) as plain files instead of a
// database or CMS. Each collection is a folder of .md files under
// src/content/. This file declares every collection and, crucially, its
// SCHEMA: the exact shape of the "frontmatter" (the YAML metadata block
// between --- fences at the top of each .md file).
//
// Why schemas matter: they are written with `zod` (imported as `z`), a
// validation library. `z.object({ title: z.string() })` means "frontmatter
// must have a title and it must be text". If a file breaks the rules the
// BUILD FAILS with a clear error, so bad content can never reach the live
// site. `.optional()` = may be missing, `.default(x)` = use x when missing,
// `z.coerce.date()` = accept a date-looking string and turn it into a real
// Date object.
//
// Pages then read a collection with `getCollection('name')` (see for example
// src/pages/insights/[slug].astro) and turn each entry into a page.
//
// This project stores post BODIES as raw HTML inside the .md files (they were
// migrated from Webflow), so templates render them with `set:html` instead of
// letting Astro convert Markdown. That is unusual; normally the body would be
// Markdown and you would render it with the <Content /> component.
//
// TypeScript note (.ts files): TypeScript is JavaScript plus type
// annotations. The build strips the types; what runs is plain JavaScript.
// ==========================================================================
...
// LEARNING: `defineCollection` couples a LOADER (where the files live, found
// via a glob pattern, ** meaning "any subfolder") with the schema described
// above. The variable name here does not matter; the NAME USED IN PAGES is
// the key it gets in the `export const collections` object at the bottom.
...
const knowledge = defineCollection({
  ...
  schema: z.object({
    ...
    appliesTo: z.string().optional(),           // e.g. "All plans" (from the Confluence template)
    prerequisites: z.string().optional(),       // what the reader needs before starting
